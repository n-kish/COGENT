...
def calculate_segment_length(fromto_str):
    """Calculate length of a segment from fromto attribute."""
    coords = [float(x) for x in fromto_str.split()]
    start = np.array(coords[:3])
    end = np.array(coords[3:])
    return np.linalg.norm(end - start)

def find_deepest_branch(body_element, current_depth=0, path=None):
    """
    Recursively find the deepest branch in the body hierarchy and collect geom elements.
    
    Args:
        body_element: Current body element in XML
        current_depth: Current depth in tree
        path: List to collect bodies along current path
        
    Returns:
        tuple: (max_depth, deepest_path, geoms_in_deepest_path)
    """
    if path is None:
        path = []
        
    path.append(body_element)
    
    # Get geom elements directly under this body
    geoms = body_element.findall("./geom")
    
    # Find all child bodies
    child_bodies = body_element.findall("./body")
    
    if not child_bodies:  # Leaf node
        return current_depth, path.copy(), geoms
    ...
    for child in child_bodies:
        depth, child_path, child_geoms = find_deepest_branch(child, current_depth + 1, path.copy())
        if depth > max_depth:
            max_depth = depth
            deepest_path = child_path
            deepest_geoms = geoms + child_geoms  # Combine geoms along path
            
    return max_depth, deepest_path, deepest_geoms

def adjust_robot_height(xml_path, save_path=None):
    """
    Read MuJoCo XML file, calculate safe starting height based on deepest branch,
    and update the position.
    
    Args:
        xml_path (str): Path to input XML file
        save_path (str, optional): Path to save modified XML. If None, overwrites input file.
    
    Returns:
        tuple: (new_height, leg_length, branch_info)
    """
    # Parse XML file
    tree = ET.parse(xml_path)
    root = tree.getroot()
    
    # Find the root body (body "0")
    body_zero = root.find(".//body[@name='0']")
    if body_zero is None:
        raise ValueError("Could not find body with name '0'")
    
    # Find the deepest branch starting from body "0"
    max_depth, deepest_path, branch_geoms = find_deepest_branch(body_zero)
    
    # Calculate total length along deepest branch
    total_length = 0
    leg_length = 0
    first_geom = True  # Flag to identify torso
    
    for geom in branch_geoms:
        if 'fromto' in geom.attrib:
            length = calculate_segment_length(geom.get('fromto'))
            total_length += length
            if not first_geom:  # Skip torso for leg length
                leg_length += length
            first_geom = False
    
    # Calculate safe starting height (1.25 × leg length)
    safe_height = leg_length * 1.25
    
    # Update position of body "0"
    current_pos = body_zero.get('pos')
    if current_pos:
        x, y, _ = current_pos.split()
        new_pos = f"{x} {y} {safe_height}"
        body_zero.set('pos', new_pos)
    ...
    # Collect branch information for reporting
    branch_info = {
        'depth': max_depth,
        'total_length': total_length,
        'leg_length': leg_length,
        'body_names': [body.get('name') for body in deepest_path]
    }
...
class Robot:
    ...
    def redesign_bodies_with_init_design(self, env_id, body_list, body, attach_loc=None, geom_type=None, parent_body=None, length_ratio=None, size_factor=None, gear_factor=None):
        ...
        #         fixed_pos_values = [0, 0, 0] 
        #         body.joints[0].node.attrib['pos'] = ' '.join([f'{x:.6f}'.rstrip('0').rstrip('.') for x in fixed_pos_values])
        #         parent_body.joints[0].node.attrib['pos'] = ' '.join([f'{x:.6f}'.rstrip('0').rstrip('.') for x in fixed_pos_values])
        #         print("parent_body.joints[0].node.attrib['pos']", parent_body.joints[0].node.attrib['pos'])

        if env_id == 'Ant-v5' or env_id == 'Swimmer-v5':
            body_start = parent_body.geoms[0].start 
            body_end = parent_body.geoms[0].end
            # print("parent body_start (parent_body.geoms[0].start)", body_start)
            # print("parent body_end (parent_body.geoms[0].end)", body_end)
            body_start_array = np.array(body_start) 
            body_end_array = np.array(body_end)
        elif env_id == 'Hopper-v5':
            body_fromto = parent_body.geoms[0].node.attrib['fromto']
            body_start_array, body_end_array = parse_fromto(body_fromto)
            # print("body_start_array", body_start_array)
            # print("body_end_array", body_end_array)
        ...
        if parent_body is None:
            ...
            if env_id != 'Hopper-v5':
                ...
                fromto_end = body_end_array
            # print("fromto_end", fromto_end)
            ...
            # print("body.geoms[0].node.attrib['fromto']", body.geoms[0].node.attrib['fromto'])
            # print("^^^^^^^^^^^^ ^^^^^^^^^^^^^ ^^^^^^^^^^^^^")
            if env_id == 'Hopper-v5':
                fixed_gear = 200
            else:
